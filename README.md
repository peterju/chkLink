...
- `data\LocalVersion.yaml`：本機版本資訊
- `run.cmd`：啟動 GUI
- `make.cmd`：編譯 GUI 執行檔
- `build_installer.ps1`：產生 Inno Setup 安裝程式
- `installer_template.iss`：Inno Setup 安裝腳本
- `make1.cmd`：包裝腳本，會轉呼叫 `build_installer.ps1`
...
CLI 版 `chklink_cli.py` 不參與 Inno Setup 安裝版的更新流程；若缺少 `data\config.yaml` 或 `data\LocalVersion.yaml`，仍會依既有邏輯在本機建立 fallback 檔案。
...
8. 掃描完成後呼叫共用核心輸出 Excel 報告，並把成功的連結存回 `data\visited_link.yaml`。

### 3. 已檢查連結快取

- GUI 上有「跳過已檢查過的網址」選項，CLI 也會讀取同一個 `skip_visited` 設定。
- 若啟用，程式會優先查 `data\visited_link.yaml`，避免對相同成功連結重複發送請求。
- 只有包含 `200` 的狀態會被寫回快取。
...
以上欄位都屬於使用者端的掃描設定。更新伺服器位置不放在 `data\config.yaml` 中，而是由 [chklink_config.py](/D:/pyTest/chkLink/chklink_config.py) 內的常數統一管理，避免一般使用者誤改。

若你是從 GitHub clone 本專案到自己的環境，通常應修改的是 [chklink_config.py](/D:/pyTest/chkLink/chklink_config.py) 內的：
...
- 首次執行時自動建立的 `data\LocalVersion.yaml`
- GUI 視窗標題顯示的預設版本
- Inno Setup 安裝程式顯示名稱
- 桌面與開始功能表捷徑名稱
...
1. 先用 `make.cmd` 產生 `standalone` 執行資料夾與遠端版本檔
2. 視需要用 `pycert.ps1` 對 `chklink.exe` 簽章
3. 再用 `build_installer.ps1` 透過 Inno Setup 封裝成安裝程式
4. 對外提供 `installer\chklink_setup.exe` 與 `installer\RemoteVersion.yaml`

也就是說，目前正式發佈品不是單一 `chklink.exe`，而是：

- `out\chklink.dist\`：GUI 的 `standalone` 多檔案執行目錄
- `installer\chklink_setup.exe`：對外提供的正式安裝程式
- `installer\RemoteVersion.yaml`：提供 GUI 檢查更新時比對版本用的遠端版本檔
...
1. 先更新 [chklink_config.py](/D:/pyTest/chkLink/chklink_config.py) 的版本與必要顯示名稱。
2. 執行 `make.cmd`，產生：
   - `out\chklink.dist\chklink.exe`
   - `installer\RemoteVersion.yaml`
3. 若這次要對外正式散佈，視需要執行 [pycert.ps1](/D:/pyTest/chkLink/pycert.ps1) 對 `out\chklink.dist\chklink.exe` 簽章。
4. 執行 `make1.cmd`。
   - 可選是否先執行 `pycert.ps1`
   - 之後會自動轉呼叫 `build_installer.ps1`
5. 產生：
   - `installer\chklink_setup.exe`
6. 實際安裝一次，確認安裝介面、捷徑、主程式啟動與版本資訊都正確。
7. 將 `installer` 目錄中的檔案上傳到下載伺服器。
...
- [chklink_config.py](/D:/pyTest/chkLink/chklink_config.py) 內的 `DEFAULT_RELEASE_BASE_URL` 已改成你的伺服器路徑
...
2. 執行安裝程式
3. 安裝完成後，程式主體、`data\LocalVersion.yaml` 與 `data\update.cmd` 會先放好
4. 第一次啟動 GUI 時：
...
3. 和本機 `data\LocalVersion.yaml` 比對版本
4. 若遠端較新：
...
1. 先讀取本機 `data\LocalVersion.yaml`。
2. 再依 [chklink_config.py](/D:/pyTest/chkLink/chklink_config.py) 中的發佈常數組合更新網址：
   - `DEFAULT_RELEASE_BASE_URL`
   - `DEFAULT_REMOTE_VERSION_FILE`
   - `DEFAULT_SETUP_FILE`
...
- CLI 版 `chklink_cli.py` 不參與 Inno Setup 安裝流程，目前仍以原始碼執行為主。
...
3. 再跑 `build_installer.ps1` 產生 `installer\chklink_setup.exe`
...
- 若未來要再調整程式內更新流程，請先確認 GUI 文案、`run_update()`、`make1.cmd` 與 `data\update.cmd` 是否整體一致。
