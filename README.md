...
- `run.cmd`：啟動 GUI
- `make.cmd`：第 1 階段，編譯 GUI / CLI 並準備版本檔
...
目前預設建置流程不會自動簽章；若需要正式簽章，請在 build 完成後另外執行 `make_sign.cmd`，它會依序簽 GUI、CLI 與對應版本的 installer。

若你是新維護者，或是要讓其他 AI / agent 工具接手這個專案，請另外參考根目錄的 [AGENTS.md](/D:/pyTest/chkLink/AGENTS.md)。
...
`make.cmd` 的定位是「第 1 階段：編譯與準備版本檔」，它會：
...
1. 先用 `make.cmd` 產生 GUI / CLI 編譯產物與版本檔
2. 再用 `make_setup.cmd` 透過 Inno Setup 封裝成對應版本的安裝程式
3. 若這次是正式對外版本，再視需要執行 `make_sign.cmd`
4. 對外提供 `installer\<版本>\chklink_setup.exe` 與 `installer\<版本>\RemoteVersion.yaml`
...
- 將最新的 `data\LocalVersion.yaml` 複製成 `installer\<版本>\RemoteVersion.yaml`
...
`make_setup.cmd` 的定位是「第 2 階段：建立 installer」，它會直接轉呼叫 `build_installer.ps1`。
...
3. 執行 `make_setup.cmd`，產生：
   - `installer\<版本>\chklink_setup.exe`
4. 若這次要對外正式散佈，再執行 `make_sign.cmd`，它會簽：
   - `out\chklink.dist\chklink.exe`
   - `out\chklink_cli.exe`
   - `installer\<版本>\chklink_setup.exe`
5. 實際安裝一次，確認安裝介面、捷徑、主程式啟動與版本資訊都正確。
6. 將 `installer\<版本>\` 目錄中的檔案上傳到下載伺服器。

也就是說，日常 build / 發版分工如下：

1. `make.cmd`：編譯 GUI / CLI，並準備版本檔
2. `make_setup.cmd`：建立 installer
3. `make_sign.cmd`：正式發佈前才執行的簽章步驟

若只是內部測試，通常跑完 `make.cmd` 與 `make_setup.cmd` 即可。
...
部分電腦或防毒產品會把新編譯出的執行檔或 installer 視為可疑檔案，甚至直接隔離或刪除。`pycert.ps1` 是用來降低這類問題的簽章腳本，目前會依版本自動尋找並處理：
...
- 使用 `signtool.exe` 對 GUI、CLI 與 installer 做 Authenticode 簽章
- 用既有憑證指紋 `/sha1 ...` 指定簽章憑證
...
1. 先跑 `make.cmd`
2. 再跑 `make_setup.cmd`
3. 若需要正式簽章，再跑 `make_sign.cmd`
4. 將新的安裝程式與版本檔提供給使用者安裝或更新
