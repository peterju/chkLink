...
- `%LOCALAPPDATA%\chkLink\data\config.yaml`：實際使用的設定檔
- `config.yaml-default`：預設設定樣板
- `%LOCALAPPDATA%\chkLink\data\visited_link.yaml`：已檢查過且回應正常的連結快取
- `run.cmd`：啟動 GUI
...
- 使用者執行期資料是 `%LOCALAPPDATA%\chkLink\data\config.yaml` 與 `%LOCALAPPDATA%\chkLink\data\visited_link.yaml`。
...
- 若缺少 `%LOCALAPPDATA%\chkLink\data\config.yaml`，會在本機建立預設設定檔
- 讀取 `%LOCALAPPDATA%\chkLink\data\config.yaml`
- 若存在則載入 `%LOCALAPPDATA%\chkLink\data\visited_link.yaml`；此檔為成功連結快取，通常會在完成一次掃描並儲存結果後建立或更新

CLI 版 `chklink_cli.py` 不參與 Inno Setup 安裝版的更新流程；若缺少 `%LOCALAPPDATA%\chkLink\data\config.yaml`，仍會依既有邏輯在本機建立 fallback 檔案。
...
自 `1.4.1` 起，使用者設定與快取會優先寫到 `%LOCALAPPDATA%\chkLink\data\`，避免安裝在 `Program Files` 時因權限不足導致 GUI 無法正常收尾或儲存設定；`update.cmd` 則固定放在安裝根目錄。
...
9. 掃描完成後呼叫共用核心輸出 Excel 報告，並把成功的連結存回 `%LOCALAPPDATA%\chkLink\data\visited_link.yaml`。

### 3. 已檢查連結快取

- GUI 上有「跳過已檢查過的網址」選項，CLI 也會讀取同一個 `skip_visited` 設定。
- 若啟用，程式會優先查 `%LOCALAPPDATA%\chkLink\data\visited_link.yaml`，並以正規化後的 URL 當 key，避免對同一頁不同寫法重複發送請求。
- 只有包含 `200` 的狀態會被寫回快取。
- `%LOCALAPPDATA%\chkLink\data\visited_link.yaml` 不一定會在首次啟動時出現；通常是在實際掃描完成並寫回快取後才建立。
...
`%LOCALAPPDATA%\chkLink\data\config.yaml` 主要欄位如下：
...
以上欄位都屬於使用者端的掃描設定。更新伺服器位置不放在 `%LOCALAPPDATA%\chkLink\data\config.yaml` 中，而是由 [chklink_config.py](chklink_config.py) 內的常數統一管理，避免一般使用者誤改。

### 關於自訂 Header 與 Cookie

- 預設設定已不再內建 `Cookie` 或其它 session/token 類值，避免公開 repo 時把敏感請求資料一併帶出。
- 若某些站台必須依賴登入態、SSO 或特定 session 才能開啟，請由使用者自行在 `%LOCALAPPDATA%\chkLink\data\config.yaml` 的 `headers` 區塊補上需要的值。
...
這樣就能把 GUI 更新按鈕改指向你自己的下載伺服器，而且不需要把部署端設定曝露在使用者的 `%LOCALAPPDATA%\chkLink\data\config.yaml` 中。
...
- GUI 與 CLI 的使用方式
- `%LOCALAPPDATA%\chkLink\data\config.yaml` 的設定格式
- Excel 報表欄位與輸出格式
- 安裝、升級與更新流程
...
- 將 `out\chklink.dist`、`out\chklink_cli.exe` 與 `update.cmd` 一起包進安裝檔
...
### 為什麼 installer 不再包含使用者設定與快取

正式路線改成 `standalone + Inno Setup` 後，安裝與升級都共用同一個 `setup.exe`。若 installer 內包含使用者設定或快取，使用者之後重新執行新版安裝程式覆蓋安裝時，就容易把：

- `%LOCALAPPDATA%\chkLink\data\config.yaml`
- `%LOCALAPPDATA%\chkLink\data\visited_link.yaml`

這些使用者自己的設定與快取一併洗掉。

因此目前採取的原則是：

- installer 內不包含使用者的 `config.yaml`
- installer 內不包含使用者的 `visited_link.yaml`
- GUI 第一次啟動時若找不到 `%LOCALAPPDATA%\chkLink\data\config.yaml`，才在本機建立預設設定檔
- `%LOCALAPPDATA%\chkLink\data\visited_link.yaml` 則維持掃描後才建立
...
- `%LOCALAPPDATA%\chkLink\data\config.yaml` 與 `%LOCALAPPDATA%\chkLink\data\visited_link.yaml` 屬於執行期檔案，不應提交。
...
2. 執行安裝程式
3. 安裝完成後，程式主體與 `update.cmd` 會先放好
4. 第一次啟動 GUI 時：
   - 若沒有 `%LOCALAPPDATA%\chkLink\data\config.yaml`，程式會在本機建立預設設定檔
   - 若沒有 `%LOCALAPPDATA%\chkLink\data\visited_link.yaml`，先不建立，等實際掃描完成後再建立
...
4. 若遠端較新：
   ...
   - 交給 `update.cmd` 啟動新版安裝程式
5. 使用者依原路徑覆蓋安裝
6. 升級完成後：
   - 程式檔更新
   - `%LOCALAPPDATA%\chkLink\data\config.yaml` 保留
   - `%LOCALAPPDATA%\chkLink\data\visited_link.yaml` 保留
...
目前 `update.cmd` 不再負責舊版那種單一 `exe` 替換，而是改成：
...
- 若未來要再調整程式內更新流程，請先確認 GUI 文案、`run_update()`、`make_exec.cmd`、`menu.cmd`、`make_sign_app.cmd`、`make_setup.cmd`、`make_sign_setup.cmd` 與 `update.cmd` 是否整體一致。
