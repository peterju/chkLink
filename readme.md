...
- 若缺少 `config.yaml`，會在本機建立預設設定檔
- 讀取 `config.yaml`
- 若存在則載入 `visited_link.yaml`；此檔為成功連結快取，通常會在完成一次掃描並儲存結果後建立或更新

CLI 版 `chklinkTerminal.py` 不參與 Inno Setup 安裝版的更新流程；若缺少 `config.yaml` 或 `LocalVersion.yaml`，仍會依既有邏輯在本機建立 fallback 檔案。
...
- `avoid_urls`：略過檢查的網址
- `scan_urls`：起始掃描網址
- `remote_version_url`：遠端版本資訊 `RemoteVersion.yaml` 的下載位置
- `setup_url`：新版 `chklink_setup.exe` 的下載位置
...
3. 再用 `make_setup.ps1` 透過 Inno Setup 封裝成安裝程式
...
`make_setup.ps1` 會先檢查 `ISCC.exe` 與 `ChineseTraditional.isl` 是否存在，任一缺少都會直接停止。
...
- 將最新的 `LocalVersion.yaml` 複製成 `installer\RemoteVersion.yaml`

這樣可以確保：

- 開發機上的 `LocalVersion.yaml`
- 安裝後程式實際使用的版本
- 下載伺服器上的 `RemoteVersion.yaml`

三者一致。

`make_setup.ps1` 則負責把 `standalone` 產物封裝成安裝程式，它會：

- 讀取 `chklink_config.py` 的 `APP_NAME`、`APP_DISPLAY_NAME`、`DEFAULT_APP_VERSION`
- 重寫 `chklink_setup.iss` 內的版本、顯示名稱與來源路徑
- 將 `out\chklink.dist`、`LocalVersion.yaml`、`update.cmd` 一起包進安裝檔
- 呼叫 Inno Setup 6 的 `ISCC.exe`
- 產出 `installer\chklink_setup.exe`

### 為什麼 installer 不再包含 `config.yaml`

正式路線改成 `standalone + Inno Setup` 後，安裝與升級都共用同一個 `setup.exe`。若 installer 內包含 `config.yaml`，使用者之後重新執行新版安裝程式覆蓋安裝時，就容易把：
...
- installer 內不包含 `config.yaml`
- installer 內不包含 `visited_link.yaml`
- GUI 第一次啟動時若找不到 `config.yaml`，才在本機建立預設設定檔
- `visited_link.yaml` 則維持掃描後才建立
...
2. 執行安裝程式
3. 安裝完成後，程式主體、`LocalVersion.yaml` 與 `update.cmd` 會先放好
4. 第一次啟動 GUI 時：
   - 若沒有 `config.yaml`，程式會在本機建立預設設定檔
   - 若沒有 `visited_link.yaml`，先不建立，等實際掃描完成後再建立
...
6. 升級完成後：
   - 程式檔與版本檔更新
   - `config.yaml` 保留
   - `visited_link.yaml` 保留

### GUI 內的「檢查更新」按鈕實際流程

目前 GUI 的「檢查更新」按鈕已直接走新版 installer 升級邏輯，實際步驟如下：

1. 先讀取本機 `LocalVersion.yaml`。
2. 再讀取 `config.yaml` 中的：
   - `remote_version_url`
   - `setup_url`
3. 下載遠端 `RemoteVersion.yaml` 到暫存目錄。
4. 比對遠端版本與本機版本。
5. 若遠端較新，詢問是否下載並啟動新版安裝程式。
6. 使用者同意後：
...
- CLI 版 `chklinkTerminal.py` 不參與 Inno Setup 安裝流程，目前仍以原始碼執行為主。
...
- 若未來要再調整程式內更新流程，請先確認 GUI 文案、`run_update()`、`make1.cmd` 與 `update.cmd` 是否整體一致。
