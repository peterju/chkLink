...
2. 啟動 headless Chrome 與 ChromeDriver。
...
8. 掃描完成後呼叫 `report()` 產生 Excel 報告，並把成功的連結存回 `visited_link.yaml`。
...
- 將版本資訊帶入執行檔屬性
- 用 Nuitka 將 `chklink.py` 打包成單檔 GUI 程式
- 產出 `out\chklink.exe`
...
- GUI 與 CLI 邏輯高度相似，但仍是各自維護，修改時要同步確認。
