...
def queued_link_check(start_url, depth_limit=1) -> list:
    ...
    queue = deque([(start_url, 0)])  # 存放待檢查的連結
    all_err_links = []  # 存放所有錯誤連結

    while queue:
        url, current_depth = queue.popleft()  # 取得待檢查的連結
        match = re.search(r".*\.\w{3,4}$", url)  # 是否符合具有副檔名的.xxx或.xxxx (例如: .html, .php, .aspx)
        if match:  # 若連結為檔案
            continue  # 跳過
        # if url.endswith(
        #     ('.pdf', '.odt', '.png', '.jpg', '.ico', '.gif', '.zip', '.7z', '.mp3', '.mp4', '.doc', '.docx')
        # ):
        #     continue  # 若連結為檔案，則跳過
        if url not in visited and current_depth <= depth_limit:  # 若連結未檢查過且深度未達到指定深度
            ...
            (
                all_err_links.extend(err_inter_links) if err_inter_links else all_err_links
            )  # 將錯誤的內部連結加入錯誤連結集合
            (
                all_err_links.extend(err_outer_links) if err_outer_links else all_err_links
            )  # 將錯誤的外部連結加入錯誤連結集合
    ...
    all_err_links = list(set(all_err_links))  # 去除重複的錯誤連結
    return all_err_links  # 回傳錯誤連結集合
...
if __name__ == "__main__":
    ...
    result = queued_link_check(start_url, 3)
    ...
    with open(csvname, 'w', newline='', encoding='Big5') as csvfile:
        ...
        writer.writeheader()
        for link in result:
            writer.writerow({'Url': link})  # 寫入錯誤連結
