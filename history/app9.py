...
# ---------------- 系統設定 ----------------
WANTED_URLS_FILE = 'urlList.txt'
AVOID_URLS_FILE = 'urlAvoidList.txt'
...
with open('headers.txt', 'r') as f:
    HEADERS = json.loads(
        json.dumps(eval(f.read()))
    )  # 定義連線標頭 https://blog.csdn.net/qq_39852676/article/details/103895273
...
def is_valid_link(base_url, link) -> bool:
    ...
    if full_url in avoid_urls:  # 若連結為避免檢查的網址
        ...
        try:
            ...
            if status == '200' and domain1 != domain2:  # 若原始連結與實際連結的網域不同
                response = requests.get(full_url, headers=HEADERS, timeout=TIMEOUT, allow_redirects=False, stream=True)
            ...
            response = requests.get(full_url, headers=HEADERS, timeout=TIMEOUT, stream=True, verify=False)
            if response.status_code == 200:
                status = "200 (忽略 SSL 錯誤)"
            else:
                status = f"SSL 錯誤：{link}  錯誤訊息：{e}"
...
def get_links(domain, url) -> tuple:
    '''取得網頁中的所有內部連結與外部連結'''
    try:
        response = requests.get(url.strip(), headers=HEADERS, timeout=TIMEOUT, allow_redirects=True)  # 發送 GET 請求
        real_url = response.url  # 取得網頁的實際連結, 避免重定向造成誤判
        ...
        for link in all_links:
            if link.startswith(('#', 'javascript', 'mailto')):  # 若連結為錨點、javascript 或郵件連結
                continue
            elif link.startswith((url, '/')):  # 若連結為內部連結
    ...
    try:
        ...
        with open(file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if line and not line.startswith('#'):  # 若為空白行或註解則跳過
                    if not line.startswith(('http://', 'https://')):  # 若網址未包含協定，則加上 https 協定
                        line = f"https://{line}"
...
# ---------------- 主程式 ----------------
wanted_urls = file_to_list(WANTED_URLS_FILE)  # 讀取 urlList.txt 檔案中想要檢查的網址
avoid_urls = file_to_list(AVOID_URLS_FILE)  # 讀取 urlAvoidList.txt 檔案中想要避免檢查的網址

for start_url in wanted_urls:
