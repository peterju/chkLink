...
def run_update():
    ...
    def wg():
        wget.download(
            'https://filedn.com/lUx2mFXVGAyFPYDUlzHuvcR/chklink/RemoteVersion.yaml',
        ...
        if local_ver.get('version') == remote_ver.get('version'):
            ...
            filename = wget.download(
                'https://filedn.com/lUx2mFXVGAyFPYDUlzHuvcR/chklink/update.7z',
                bar=update_progress_bar,
            )
            with SevenZipFile(filename, 'r') as archive:
                archive.extractall()  # 解壓縮
            os.replace('update_upd.exe', 'update.exe')  # 更新舊版 update.exe
            os.replace('RemoteVersion.yaml', 'LocalVersion.yaml')  # 更新版本記錄
            os.remove(filename)
            subprocess.Popen(["update.exe"])  # 啟動更新程式

    if os.path.isfile('appui_upd.exe'):
        os.remove('appui_upd.exe')
...
def dl_resources():
    '''更新必要的元件'''
    URL = 'https://filedn.com/lUx2mFXVGAyFPYDUlzHuvcR/chklink/resources.7z'
...
if os.path.exists(config_file):
    setting = read_config(config_file)  # 讀取設定檔
else:
...
form.title(f"網頁失效連結掃描工具 Ver.{version}")
...
Hovertip(start_url_txt, '輸入想要掃描的網址，若有多個網址請以逗號分隔')
...
log_console = scrolledtext.ScrolledText(frame1_2, wrap=tk.WORD, font=('Courier New', 10))
