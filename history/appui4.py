...
def choose_dir():
    ...
    if folder_selected:  # 若有選擇目錄
        ...
        DOC_FOLDER = dl_folder  # 設定全域變數 DOC_FOLDER
...
def read_config() -> dict:
    ...
    try:
        ...
        setting['timeout'] = 3  # 定義連結檢查層數
        setting['alt_must'] = False  # 定義是否必須偵測圖片的 alt 屬性
        setting['headers'] = {
...
if setting.get('rpt_folder'):
    DOC_FOLDER = setting.get('rpt_folder')  # 定義報告檔案目錄
else:
    DOC_FOLDER = os.path.join(os.environ['USERPROFILE'], 'Documents')  # 預設檔案存放目錄為【我的文件】
...
# start_url_txt.config(height=len(SCAN_URLS) // 5)
...
report_dir_txt.insert(0, DOC_FOLDER)  # 設定預設下載目錄
