...
# ---------------- 使用者設定 ----------------
WANTED_URL = open('urlList.txt', 'r', encoding='utf-8').read().splitlines()  # 讀取 urls.txt 檔案中想要檢查的網址
AVOID_URL = (
    open('urlAvoidList.txt', 'r', encoding='utf-8').read().splitlines()
)  # 讀取 urlAvoidList.txt 檔案中想要避免檢查的網址
LAYER = 3  # 定義檢查連結的層數
TIMEOUT = 8  # 定義連線逾時秒數
...
def is_valid(url, link) -> bool:
    ...
    if fqdn in AVOID_URL:  # 若連結為避免檢查的網址
        status = "200 (避免檢查的網址)"
    else:
...
def create_logger(log_folder, filename) -> logging.Logger:
    """建立 logger"""
    logger = logging.getLogger()  # 取得 logger
...
# ---------------- 主程式 ----------------
loggers = []  # 建立一個空列表來追蹤所有的logger
for start_url in WANTED_URL:
    start_url = start_url.strip()  # 去除 url 的前後空白
    if start_url[0] == '#':  # 若為註解則跳過
        continue
    if not start_url.startswith(('http://', 'https://')):  # 若網址未包含協定，則加上 https 協定
    ...
    loggers.append(logger)  # 將建立的logger加入列表中
