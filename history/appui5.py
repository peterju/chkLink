...
def resource_path(relative_path):
    '''取得資源的絕對路徑，適用於 PyInstaller'''
    # https://stackoverflow.com/questions/73849342/packing-images-together-with-the-main-file-with-pyinstaller-onefile-problem
    # https://www.jb51.net/article/271076.htm
...
def queued_link_check(start_urls, depth_limit=1) -> list:
    '''使用雙向佇列結構儲存網站中的連結'''
    global logger, HEADERS, AVOID_URLS
    # 取得設定檔的標頭與避免檢查的網址
    HEADERS = dict()
...
if os.path.exists(config_file):
    ...
    print(f"首次執行，為您建立設定 {config_file}...")
...
# 建立網址輸入框
start_url_txt = ttk.ScrolledText(frame1_1, wrap=tk.WORD, height=5)
...
Hovertip(start_url_txt, '輸入想要掃描的網址，多個網址以逗號分隔')
...
# 建立 frame1_2
frame1_2 = ttk.Labelframe(page1, text="掃描歷史", style='success.TLabelframe')
...
frame2_1.columnconfigure(7, weight=2)
frame2_1.columnconfigure(9, weight=1)
...
# 建立下載目錄Label
report_dir_txt_label = ttk.Label(frame2_1, text="結果存放")
report_dir_txt_label.grid(row=0, column=6, padx=5, pady=5)

# 建立下載目錄輸入框
report_dir_txt = ttk.Entry(frame2_1)
report_dir_txt.grid(row=0, column=7, padx=5, pady=5, sticky="ew")
report_dir_txt.insert(0, setting.get('rpt_folder'))  # 設定預設下載目錄

# 建立更改目錄按鈕
choose_dir_icon = tk.PhotoImage(file=resource_path('icon\\folder.png'))  # 設定更改目錄按鈕的圖示
...
choose_dir_button.grid(row=0, column=8, padx=10)

# 建立儲存設定按鈕
cfg_save_btn = ttk.Button(frame2_1, text="儲存設定", command=save_config, bootstyle="info", cursor='hand2')
cfg_save_btn.grid(row=0, column=9, padx=5, sticky="e")
