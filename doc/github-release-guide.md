...
- 如果你在 GitHub 網站建立 Release 時順手建立新 tag，本質上等於在遠端建立了一個新的 Git tag。
- 遠端新建的 tag 不會自動出現在本機 repo；本機必須再做同步。
- `git pull` 主要同步 branch；若 tag 是在網站建立的，實務上建議直接做 `git fetch --tags origin`。
...
- 本專案整理 GitHub Release 發佈檔案的腳本是 [chkLink/make_github_release.cmd](chkLink/make_github_release.cmd)。

## 建立 Release 前的前置確認

在建立 GitHub Release 前，先確認這幾件事：

1. [chkLink/chklink_config.py](chkLink/chklink_config.py) 的 `DEFAULT_APP_VERSION` 已是目標版本。
2. `main` 已包含本次版本要公開的 commit。
3. 本機工作樹乾淨，避免把未完成內容誤認為已發佈狀態。
4. `installer\<version>\` 內的安裝檔、`RemoteVersion.yaml`、`SHA256.txt` 已準備完成。
5. `release\<version>\` 內的 GitHub Release 發佈檔案已由 [chkLink/make_github_release.cmd](chkLink/make_github_release.cmd) 整理完成。
...
4. 建立新 Release 時，直接選既有 tag `v<version>`。
5. 上傳 `release\<version>\` 內整理好的對外發佈檔案。
...
3. 在 `Choose a tag` 輸入例如 `v<version>`。
4. GitHub 會讓你建立新 tag。
5. 選擇 `Target`，通常是 `main`。
6. 若 GitHub 畫面有 `Previous tag`，可選前一版 tag 讓 GitHub 比較版本差異或產生 release notes。
...
10. 若只是先整理內容，可先存成 draft；確認檔案齊全後再正式發佈。
...
- `git tag -l v<version>` 有結果，代表本機已看得到該 tag。
- `git ls-remote origin refs/tags/v<version>` 有結果，代表 GitHub 遠端有該 tag。
...
1. 完成程式修改與驗證。
2. 確認版本號已升級。
3. 完成 build、sign、setup、SHA256、GitHub Release 發佈檔案整理。
